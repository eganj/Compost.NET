...
gulp changelog > debug.txt
pause

:: Publish to GitHub releases
set /p API_KEY=< GitHubApiKey.txt
set /p VERSION=< release_version.txt  
GithubReleaseCreator_1.1.0\GithubReleaseCreator.exe --username %API_KEY% --owner "eganj" --repo "compost.net" --name %VERSION% --filedesc "CHANGELOG.md" -assetfiles "bin\Release\Compost.dll,bin\Release\Compost.pdb,bin\Release\Compost.XML" --verbose
pause
